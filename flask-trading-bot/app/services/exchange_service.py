...
class ExchangeService:
    ...
    def _init_exchange(self) -> None:
        ...
        try:
            ...
            if self._dry_run:
                # No usar sandboxMode para evitar problemas de conexión pública
                pass
    ...
    def create_order(
        ...
        try:
            if self._dry_run:
                logger.info(
                    f"[DRY RUN] Orden {side} {order_type}: "
                    f"{amount} {pair} @ {price if price else 'market'}"
                )
                return {
                    'id': f'dry_run_{pair}_{side}',
                    'symbol': pair,
                    'type': order_type,
                    'side': side,
                    'amount': amount,
            ...
            logger.info(f"Orden creada: {order['id']} - {side} {amount} {pair}")
            return order
    ...
    def cancel_order(self, order_id: str, pair: str) -> dict[str, Any]:
        ...
        try:
            if self._dry_run:
                logger.info(f"[DRY RUN] Cancelando orden {order_id}")
                return {'id': order_id, 'status': 'canceled'}
            
            order = self.exchange.cancel_order(order_id, pair)
